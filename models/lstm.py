...
w = csv.writer(open('../datasets/lstm30_curr1.log','wb'))
...
for epoch in range(200):
    ...
    if epoch % 10 == 0:
        for exp in experiment_samples.keys()[14:]:
            ...
            plt.plot(experiment_samples[exp][:,3])
            plt.savefig('../datasets/results_lstm30/curr1_' + exp + '_epoch' + str(epoch) + '_true.png')
            plt.close()
            plt.plot(pred_curr1)
            plt.savefig('../datasets/results_lstm30/curr1_' + exp + '_epoch' + str(epoch) + '_pred.png')
            plt.close()
