...
for epoch in range(200):
    ...
    for i in range(0, train.shape[0], 256):
        ...
        inp = Variable(torch.from_numpy(inp).type(torch.FloatTensor).cuda(0))
        out_true = Variable(torch.from_numpy(out_true).type(torch.FloatTensor).cuda(0))
        model.zero_grad()
        out_pred = model(inp)
