...
RefSpeed = Data.RefSpeed.Data / (2*pi); RefLoad = Data.RefLoad.Data;
Voltage = Data.Voltage.Data; t = Data.Voltage.Time;
StatorPuls = Data.StatorPuls.Data / (2*pi); %t = Data.StatorPuls.Time;
Current = Data.Current.Data; %t = Data.Current.Time;
Speed   = Data.Speed.Data / (2*pi);   %t = Data.Speed.Time;
Torque  = Data.Torque.Data;  %t = Data.Torque.Time;

figure; set(gcf,'Name','dq')
subplot(4,4,1); hold on; grid on; zoom on; xlabel(' t(s) '); ylabel(' Ref Speed (Hz) '); plot( t , RefSpeed );
subplot(4,4,2); hold on; grid on; zoom on; xlabel(' t(s) '); ylabel(' Voltage (V) '); plot( t , Voltage );
subplot(4,4,3); hold on; grid on; zoom on; xlabel(' t(s) '); ylabel(' Speed (Hz) '); plot( t , Speed );
...
subplot(4,4,6); hold on; grid on; zoom on; xlabel(' t(s) '); ylabel(' StatorPuls (Hz) '); plot( t , StatorPuls );
