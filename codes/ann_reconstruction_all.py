...
class FFNet(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.linear4 = nn.Linear(120, input_dim)
        self.dropout = nn.Dropout(0.5)

    def forward(self, seq):
        x = seq.view(seq.size()[0], -1)
        out = F.relu6(self.linear1(x))
        out = F.relu6(self.dropout(self.linear2(out)))
        out1 = self.linear3(out)
        out2 = self.linear4(out)
        return out1, out2


model_current1 = FFNet(window*3, window).cuda()
model_current2 = FFNet(window*3, window).cuda()
model_torque = FFNet(window*3, window).cuda()
...
for epoch in range(num_epochs):
    ...
    for i in range(0, train.shape[0], batch_size):
        ...
        current1_pred1, current1_pred2 = model_current1(inp)
        current1_loss = loss_function(current1_pred1, current1_true) + \
                            loss_function(current1_pred2, inp.view(inp.size()[0], -1))
        current1_loss.backward()
        optimizer_current1.step()

        model_current2.zero_grad()
        current2_pred1, current2_pred2 = model_current2(inp)
        current2_loss = loss_function(current2_pred1, current2_true) + \
                            loss_function(current2_pred2, inp.view(inp.size()[0], -1))
        current2_loss.backward()
        optimizer_current2.step()

        model_torque.zero_grad()
        torque_pred1, torque_pred2 = model_torque(inp)
        torque_loss = loss_function(torque_pred1, torque_true) + \
                        loss_function(torque_pred2, inp.view(inp.size()[0], -1))
...
torch.save(model_current1, '../weights/SE_data_current1_ann_recon' + str(window) + '.pt')
torch.save(model_current2, '../weights/SE_data_current2_ann_recon' + str(window) + '.pt')
torch.save(model_torque, '../weights/SE_data_torque_ann_recon' + str(window) + '.pt')
